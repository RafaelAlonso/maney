...
RSpec.describe "CardMigrations", type: :request do
  ...
  it "shows the counts and the destinations (AC 17)" do
    ...
    # Anchored on word boundaries so a scope bug that counts the 10 parcela
    # rows alongside the 1 avulso (11 total) can't satisfy this by rendering
  ...
  it "discloses that migrating re-files bills onto the destination card's faturas (Fix 1)" do
    get new_card_migration_path(card)
  ...
  it "states the fatura consequence and the counts in the migrate button's confirm (Fix 1)" do
    get new_card_migration_path(card)
  ...
  it "deletes everything and the card (AC 17)" do
    ...
    # The shipped code cascades to the purchase's 10 parcela Expense rows via
