...
RSpec.describe "Budgets (inline edit)", type: :request do
  ...
  it "creates a Budget and streams the dashboard budget editor (AC 8)" do
    ...
    # The budget edit lives on the category dashboard now, so a save streams that
    # screen's budget editor (dom_id(category, :budget)) back with the new figure.
    expect(response.body).to include(ActionView::RecordIdentifier.dom_id(mercado, :budget))
                        .and include("orçado R$ 900,00")
  end
